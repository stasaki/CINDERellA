...
% Build 1.2.0
% The original script has been modified to be able to load binary matrix.
...
classdef SlicedData < handle
    ...
    methods
        ...
        % edited by s.t
        function SamplePermute(obj)
            permind = randperm(obj.nCols);
            obj.columnNames = obj.columnNames(permind);
            for sl=1:obj.nSlices
                obj.dataSlices{sl} = obj.dataSlices{sl}(:,permind);
            end;
        end;
        % end
        ...
        function LoadBFile(obj, data,names)
            nCols = size(data,2);
            obj.columnNames = strcat('s',num2cellstr(1:nCols));%patient ids
            
            
            nRows = size(data,1);
            nSlices=ceil(nRows/obj.fileSliceSize);
            % initial length is 1
            obj.dataSlices = cell(nSlices,1);
            obj.rowNameSlices = cell(nSlices,1);
            
            k=1;
            for i=1:length(obj.dataSlices)
                obj.dataSlices{i}=data(k:min([k+obj.fileSliceSize-1 nRows]),:);
                obj.rowNameSlices{i}=names(k:min([k+obj.fileSliceSize-1 nRows]));
                k=min([k+obj.fileSliceSize-1 nRows])+1;
            end
        ...
        function LoadFile(obj, filename)
            ...
            for i=1:obj.fileSkipRows %remove colomn names
                line = fgetl(fid);
            end;
