function [sampled_graphs, LL_retain] = learn_struct_mcmc_gibbs_block4(varargin)
%learn_struct_mcmc_gibbs_block4 learning network structure by mcmc
% INPUTS:
...
mlock
persistent mar4code mar4code1 mar4code2 mar4code3 net4code
if isempty(mar4code)    
    load('net4code.mat');
end
...
% [marcoden,net4code,mar4code]=code_gen(4);
% mar4code1=marcoden{1};
% mar4code2=marcoden{2};
% mar4code3=marcoden{3};
% clear marcode;
% save('net4code','net4code','mar4code','mar4code1','mar4code2','mar4code3');
...
t_inside_max = ceil(n/4);
even_node_flag = mod(n,4)~=0;
...
while 1
    ...
    for t_inside = 1:t_inside_max
        ...
        elseif even_node_flag && order_indx==n-1
            ...
            gibbs_block2_sub
        elseif even_node_flag && order_indx==n-2
            node = update_order(order_indx:order_indx+2);
            gibbs_block3_sub
        else
            node = update_order(order_indx:order_indx+3);
            gibbs_block4_sub            
        end
